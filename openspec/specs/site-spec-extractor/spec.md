...
Define the capability that transforms a natural language prompt into a structured SiteSpec object. This is the entry point of the build pipeline — the bridge between human intent and deterministic execution. The extractor uses the LLM to interpret the prompt and produce a validated SiteSpec that downstream skills can consume.
